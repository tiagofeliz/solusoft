<div class="col-lg-3 col-md-12" id="menu">
    <div class="text-center menu menu-titulo">
        <h4><i class="fas fa-pen-square"></i> Editando Pedido</h4>
    </div>
    <div class="text-center menu menu-opcoes">
        ...
        <a href="<?php echo site_url('pedidos'); ?>" role="button" class="btn btn-danger btn-block btn-tr"><i class="fas fa-arrow-left"></i> Cancelar Edição</a>
    </div>
</div>
...
<script>
    ...
    const PEDIDO_ID = <?php echo $pedido->id; ?>
    ...
    const buscarPedido = id => {
        ...
        .done(function(response){
            ...
            selecionarCliente(pedido.cliente_id, pedido.nome)

            // inserindo produtos
            pedido.produtos.forEach(function (produto){
                produto.quantidade = parseInt(produto.quantidade)
                selecionarProduto(produto.produto_id, produto.nome, produto.cor, produto.tamanho, produto.valor, produto.quantidade, produto.id)
            })

            // demais dados
            $('#data').val(formataData(pedido.data, 'dd/mm/yyyy'))
            $('#forma_pagamento').val(pedido.forma_pagamento)
    ...
    const selecionarProduto = (id, nome, cor, tamanho, valor, quantidade = 0, itens_pedidos_id = null) => {
        ...
        $("#produtos tbody").append(`
            <tr id="produto-${index_produto}" data-index-produto="${index_produto}">
                ...
                <td><input type="number" onchange="alterarValor(this, ${index_produto})" class="text-center form-control apenas-numeros" value="${quantidade}" /></td>
                <td class="total-produto">${adicionarMascaraDinheiro(quantidade * valor)}</td>
                <td>
    ...
    const editarPedido = pedido => {
        $.ajax({
            url : "<?php echo site_url('pedidos/atualizar'); ?>",
            type : 'post',
